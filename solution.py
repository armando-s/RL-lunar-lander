...
class MLPActorCritic(nn.Module):
    ...
    def step(self, state):
        ...
        with torch.no_grad():
            ...
            return action[0].item(), v.item(), log_prob[0].item()
...
class VPGBuffer:
    ...
    def get(self):
        ...
        # Normalize the TD-residuals in self.tdres_buf
        self.tdres_buf = (self.tdres_buf - self.tdres_buf.mean()) / (self.tdres_buf.std() + 1e-16)
...
class Agent:
    ...
    def train(self):
        ...
        for epoch in range(epochs):
            ...
            # This is the end of an epoch, so here is where the policy and value functions are updated
            data = buf.get()

            # Do 1 policy gradient update
            pi_optimizer.zero_grad() # reset the gradient in the policy optimizer

            # Forward pass for all state-action-pairs to actually get gradients
            pis, log_probs = self.ac.pi(data['obs'], act=data['act'])
            loss_pi = -(log_probs * data['tdres']).sum()
            loss_pi.backward()
            pi_optimizer.step()
            ...
            for _ in range(100):
                ...
                # Targets should be the discounted rewards
                v = self.ac.v(data['obs'])
                v_loss = torch.nn.SmoothL1Loss(reduction='mean')(v, data['ret'])
                v_loss.backward()
                v_optimizer.step()
